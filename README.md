...
- WebSocket 出站握手走与 HTTPS 同一套 wreq Chrome TLS（同一 ClientHello 预置）。Upgrade 由 wreq 的 websocket 构建器完成，不走 rustls、不加站点白名单、不关 TLS 解密。帧仍由 ShowNet 转发并落库。不宣称位级 JA3 克隆。
