...
#### 内嵌浏览器启动、Cloudflare 验证与语言设置（0.4.16 新增）

- **启动不再弹出本地 Chrome 窗口。** 内嵌浏览器改用跨 Chrome 版本兼容的无头启动参数；若 Chrome 在 CDP 连接前退出，ShowNet 会立即显示明确错误，不再一直停在“正在连接”。
- **Cloudflare 真人验证可切换兼容模式。** ShowNet 检测到挑战页后会显示“兼容模式重试”。点击后仅对当前验证域名临时使用 Chrome 原生端到端 TLS，并关闭页面 Hook；业务主站与其它域名仍正常抓包。临时设置在停止浏览器或停止抓包时自动清理，不会写入永久 HTTPS 设置。
