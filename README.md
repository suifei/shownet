...
| 你想做什么 | 从这里开始 |
|------------|------------|
| 先判断 ShowNet 能做什么 | [核心能力](#核心能力重点) 与 [功能全景与工作流](docs/feature-map.md) |
| 只抓浏览器网页 | [零配置开始抓包](#1-零配置内嵌浏览器开始抓包不必先装证书) |
| 抓 App / 手机 HTTPS | [安装 CA 与设备代理](#2-需要解密-app--系统-https-时自动装证书) |
| 使用 AI Agent / Grok | [Agent/Grok 环境确认与安装](#使用-ai-agent-前先确认-grok-与网络环境) |
| 解决直连、海外代理或 `127.0.0.1:8080` 配置疑问 | [安装代理是可选项](#安装代理是可选项) |
| 从抓包生成算法或客户端代码 | [自动生成逆向 / 客户端代码](#4-自动生成逆向--客户端代码) |
| 查看 TLS、PX、JA4 与防护证据 | [高级控制台](#5-高级控制台可选增强) 与 [ClientHello 文档](docs/clienthello-catalog-and-mitm-console.md) |
| 参与开发或运行测试 | [开发](#开发) |

## 当前版本迭代（v0.4.26）

当前发布版：[ShowNet v0.4.26](https://github.com/suifei/shownet/releases/tag/v0.4.26)。这次迭代的重点
不是增加一个孤立按钮，而是把 **Agent 安装、运行边界、网络出口和用户已有环境** 的关系理顺：

| 迭代方向 | 已完成的变化 | 对用户的实际影响 |
|----------|--------------|------------------|
| Grok 来源 | ShowNet 不再编译或捆绑 Grok，改为探测系统安装，并调用 x.ai 官方 stable 安装器 | 发布包更轻；Grok 更新跟随官方；已有系统安装可以直接复用 |
| 一键安装 | 默认直连下载，ShowNet 出口代理改为可选复选框 | 没有海外代理也可以直接尝试安装；直连失败时再配置代理 |
| Agent 网络 | 安装/更新代理与 Agent 运行代理分成两个独立开关 | 可以只给安装过程使用代理，也可以只给 Agent 运行使用代理，互不影响 |
...
1. **确认自己的 Grok 环境。** 先到「设置 → AI 模型 → Agent 运行时」点击刷新，已有 Grok 就直接使用；没有再考虑一键安装。
2. **确认网络出口。** 一键安装默认直连；只有直连失败时，才到「设置 → 抓包与 HTTPS → 出口代理与 TLS 指纹」配置并探测代理。
3. **确认是否注入代理。** 安装/更新代理和 Agent 运行代理是两个开关，按实际需要分别打开，不要把系统代理或 `HTTP_PROXY` 当成 ShowNet 出口配置。
4. **确认全局环境没有被接管。** ShowNet 内的端点、MCP、Skill 和代理只对 ShowNet 启动的 Agent 生效；在系统终端独立运行 Grok 时，仍使用 Grok 自己的全局配置。

详细步骤、平台路径、Windows SOCKS5 限制和故障排查见 [Agent/Grok 环境确认与安装](#使用-ai-agent-前先确认-grok-与网络环境)。
...
### 使用 AI Agent 前：先确认 Grok 与网络环境

ShowNet 的 AI Agent 优先使用你系统中已经安装的 Grok。如果本机没有可用的 Grok，
可以在 **设置 → AI 模型 → Agent 运行时** 点击「一键安装」，由 ShowNet 下载并运行
x.ai 官方 stable 安装器：

| 平台 | 官方安装器 | 官方安装位置 |
|------|------------|--------------|
| macOS / Linux | [`https://x.ai/cli/install.sh`](https://x.ai/cli/install.sh) | `~/.grok/bin/grok` |
| Windows | [`https://x.ai/cli/install.ps1`](https://x.ai/cli/install.ps1) | `%USERPROFILE%\.grok\bin\grok.exe` |

ShowNet 不编译、不捆绑 Grok，也不会把 Grok 的二进制放进自己的发布包。安装由 x.ai
官方安装器负责，包括官方约定的用户 PATH 和安装元数据。ShowNet 只负责探测、调用和
在应用内为本次 Agent 运行准备隔离环境。

#### 安装前检查清单

第一次点击「一键安装」前，请按下面顺序确认当前环境：

1. **确认系统和架构。** 当前发布包是 macOS Apple Silicon 和 Windows x64 版本；
   如果你使用其他系统或架构，请先按 x.ai 官方方式安装，再在 ShowNet 中点击「选择文件」。
2. **先重新探测本机。** 在「Agent 运行时」点击刷新。若状态显示已找到 Grok，
   直接使用即可，不需要再次安装。也可以用「选择文件」指定你自己安装的 Grok 可执行文件。
3. **确认 AI 端点和凭据。** 在同一页配置 ShowNet 要使用的 AI 提供商、端点和 API Key。
   这些设置属于 ShowNet 的应用内配置，不会覆盖 Grok 在系统全局环境中的端点或凭据。
4. **确认网络方式。** 默认使用直连下载，不要求用户必须使用海外代理。能够直连 x.ai
   的环境不需要额外配置；只有直连失败或当前网络明确要求代理时，才需要继续配置下面的
   ShowNet 出口代理。

#### 安装代理是可选项

安装区域中的「**下载与检查更新时使用 ShowNet 出口代理**」默认关闭。它只决定
x.ai 官方安装器下载脚本和检查版本时的网络出口，不是安装的前置条件。

- **默认直连：** 不使用 ShowNet 出口代理，直接访问 x.ai 官方地址。
- **启用代理：** 先到「设置 → 抓包与 HTTPS → 出口代理与 TLS 指纹」配置并保存，
  再勾选安装区域中的选项。ShowNet 只会注入这份已保存的出口代理配置。
- **没有配置代理：** 一键安装仍然可以点击；ShowNet 只提示你在直连失败后再配置代理。
- **Windows 的 SOCKS5：** x.ai 官方 Windows PowerShell 安装器不支持 SOCKS5，
  此时请使用直连，或把 ShowNet 出口代理配置为 HTTP/HTTPS。

这里有三个容易混淆的网络概念：

| 配置 | 用途 | 是否自动影响 Grok |
|------|------|-------------------|
| ShowNet 抓包入口（默认 `127.0.0.1:8888`） | 接收浏览器、设备或脚本的被抓流量 | 否 |
| ShowNet 出口代理 | ShowNet 访问外网，以及用户选择后用于 Grok 安装/更新或 Agent 运行 | 否，必须逐项勾选 |
| 系统代理 / shell 中的 `HTTP_PROXY` 等环境变量 | 用户自己的系统或命令行网络配置 | 否，ShowNet 不会直接继承给 Agent |

例如，你本机可能有一个外网代理监听在 `127.0.0.1:8080`，也可能使用其他地址；
请以「出口代理与 TLS 指纹」页面中保存并探测成功的配置为准，不要把它和 ShowNet
抓包端口 `8888` 混用。

#### 推荐安装流程

1. 打开 **设置 → AI 模型 → Agent 运行时**，点击刷新，确认是否已经存在 Grok。
2. 已存在时，检查显示的版本和可执行文件路径；路径正确即可直接开始 AI 分析。
3. 未安装时，保持「下载与检查更新时使用 ShowNet 出口代理」不勾选，先尝试「一键安装」。
4. 直连下载失败时，记录错误提示；到「出口代理与 TLS 指纹」配置并保存可用的
   HTTP/HTTPS 代理，再返回 Agent 运行时勾选安装代理并重试。
5. 安装完成后点击刷新，确认状态显示 Grok 版本、可执行文件路径和可用状态。
6. 发起一次范围较小的 AI 分析，确认端点、MCP、Skill 和报告链路都能正常工作。

如果你之前通过「选择文件」使用了一个手动安装的 Grok，之后点击官方安装器的「安装并切换」
会切换到官方 stable 的 `~/.grok/bin/grok`。原来手动选择的文件不会被删除或修改。

#### 安装代理与 Agent 运行代理彼此独立

安装完成后，下面另有「**Agent 使用 ShowNet 出口代理**」开关。它只影响由 ShowNet
启动的 Agent 子进程，默认关闭；它与上面的安装/更新代理选项相互独立：

- 安装时可以使用代理，日常 Agent 运行仍然直连。
- 安装时可以直连，日常 Agent 运行时按需使用代理。
- 两项都可以关闭，完全不向 Agent 注入 ShowNet 出口代理。
- ShowNet 会为应用内启动的 Agent 临时注入当前 AI 端点、凭据、Skills、MCP、白名单和
  用户选择的代理；Agent 退出后，这些设置不会写回 Grok 的全局配置。
- Grok 在 ShowNet 外部由用户自己启动时，继续使用它自己的全局端点、工具、Skill、MCP
  和代理配置；ShowNet 不负责替用户配置外部运行环境。

#### 安装后如何确认环境正确

完成安装后，建议检查以下四项：

- Agent 运行时状态显示可用版本，并且路径指向你预期的 Grok 可执行文件。
- 需要代理时，安装/运行对应的开关已明确勾选；不需要代理时保持关闭。
- ShowNet 内的 AI 分析能读取当前会话并生成带证据回链的报告。
- 关闭 ShowNet 后，在系统终端直接运行 Grok，确认它仍保留原来的全局配置；
  ShowNet 不应因为一次应用内分析而改变它。

#### 常见情况

- **直连安装失败：** 不代表 Grok 安装器有问题。先确认能访问官方地址，再配置并探测
  ShowNet 出口代理后重试。
- **找不到已经安装的 Grok：** 点击刷新；仍找不到时用「选择文件」选择真实可执行文件。
  ShowNet 会先验证文件，再保存路径。
- **想恢复自动探测：** 在已选择手动路径时点击「改用自动探测」。
- **只想运行自己环境的 Grok：** 可以完全不使用 ShowNet 的一键安装；选择本机文件后，
  ShowNet 只在应用内启动它，不接管它离开 ShowNet 后的全局环境。
- **没有海外代理：** 不需要为了安装强行配置代理。先用默认直连，只有实际访问失败时
  才启用代理。
...
先按上面的 [Agent/Grok 环境确认流程](#使用-ai-agent-前先确认-grok-与网络环境) 完成运行时检查，
再在「**AI 分析**」中选择 **JS 加密逆向** 或 **API 逆向** 等模式，限定请求范围后启动。
