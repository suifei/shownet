...
The analysis Graph is an advisory route and audit model. It records the suggested Skill sequence, actual dynamic branches, retries, allowed MCP calls, output-contract results, and final report gate. Grok may change reasoning order and revisit evidence, but the runtime boundary remains enforced independently of Graph routing: local file, terminal, and external web tools are denied, and only the Skill plan's ShowNet MCP tools are available.
...
ShowNet invokes a compatible system Grok; it does not compile or bundle one. Each run uses a private temporary `GROK_HOME`, a cleared and minimally rebuilt environment, denied local Bash/file/web tools, and a short-lived analysis-scoped MCP token. Upstream-supported macOS/Linux builds also receive Grok's strict OS sandbox; Windows keeps the common denied-tool and MCP boundary without claiming an unavailable kernel sandbox. Endpoint, credentials, Skills, MCP, and optional proxy values exist only in the child process and never alter global Grok configuration.
...
The system Grok calls ShowNet's loopback MCP endpoint with a short-lived token bound to one analysis and Session. It advertises and executes only the Skill plan's allowed tools; resources, resource templates, and prompts are unavailable to this token, and `analysisId` must exactly match the active analysis. The long-lived global MCP token retains the user-configured surface. Calls are attached to the current Graph for audit. A short per-analysis lock serializes only Graph JSON updates, so concurrent allowed calls cannot overwrite each other's traces. The server always requires a bearer token, accepts only local Origins, caps request bodies, and never binds beyond loopback.
...
- Implemented: compatible system Grok discovery plus user-triggered official installation, exact user-configured `--max-turns`, per-analysis private configuration, environment-only credentials, denied host file/terminal/web tools, Skill-scoped ShowNet MCP access, strict OS sandbox on supported macOS/Linux, scaled watchdog, cancellation, cleanup, live audit events, and a native OpenAI-compatible fallback when Grok is absent.
