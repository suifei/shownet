...
if (!options.stampOnly) {
  ...
  const windowsLinkEnv = windowsTarget
    ...
    : {};
  if (windowsTarget) {
    console.log(`Windows release link env: RUSTFLAGS=${windowsLinkEnv.RUSTFLAGS}`);
  }
  if (options.xwin) {
 ...
 * Written under the sidecar checkout (ephemeral; reset on next ensurePinnedSource).
 */
async function writeWindowsReleaseCargoConfig(sourceDir) {
  const cargoDir = join(sourceDir, ".cargo");
  await mkdir(cargoDir, { recursive: true });
  const configPath = join(cargoDir, "config.toml");
  const body = `# Generated by build-grok-sidecar.mjs — avoid MSVC LNK4319 PDB limit
[profile.release]
debug = 0
strip = "symbols"
incremental = false
...
rustflags = ["-C", "debuginfo=0", "-C", "link-arg=/DEBUG:NONE", "-C", "link-arg=/INCREMENTAL:NO"]

[target.aarch64-pc-windows-msvc]
rustflags = ["-C", "debuginfo=0", "-C", "link-arg=/DEBUG:NONE", "-C", "link-arg=/INCREMENTAL:NO"]
`;
  await writeFile(configPath, body, "utf8");
  console.log(`Wrote Windows release cargo config: ${configPath}`);
}
