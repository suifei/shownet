...
fn analysis_messages(
    ...
    {
        "\n\n算法还原与重播要求：目标是把抓包+Hook+代码片段中的算法还原成可实现流水线，而不是空骨架。报告必须包含章节「算法还原」并尽量嵌入 fenced ```algorithm-spec``` JSON（reconstructionMode/confidence/algorithms/pipeline[]，每步含 name/status/formula/evidence）。status 仅可为 reconstructed|partial|trace_driven|insufficient。对 VMP/字节码/控制流平坦化/魔改 JS 必须标 vmp_hybrid 与 trace_driven，禁止假装完成完整 VM 反编译。随后调用 shownet_build_algorithm_replay（默认 python 或用户语言）物化 ALGORITHM_SPEC 与可运行重播；写入可用时用 shownet_export_analysis_artifacts 落盘。密钥/token 只允许 env；并用 validate 清单说明如何对照抓包字段形状做授权目标前自测。"
    } else {
        ""
    };
