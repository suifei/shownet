...
describe("embedded browser language", () => {
  it("canonicalizes a freely entered BCP 47 language", () => {
  ...
  it("lets Chrome generate HTTP weights from its unweighted profile preference", async () => {
    const view = await readFile(new URL("../src/components/BrowserView.tsx", import.meta.url), "utf8");
    const overrideStart = view.indexOf('send("Emulation.setUserAgentOverride"');
    const overrideEnd = view.indexOf("});", overrideStart);
    assert.ok(overrideStart >= 0 && overrideEnd > overrideStart, "the UA override must remain present");
    assert.doesNotMatch(
      view.slice(overrideStart, overrideEnd),
      /acceptLanguage\s*:/,
      "CDP must not receive an already weighted HTTP value; Chrome owns Accept-Language generation",
    );
    assert.doesNotMatch(view, /browserAcceptLanguage/);
  });
});
