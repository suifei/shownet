...
describe("system Grok integration boundaries", () => {
  it("keeps official installation user-triggered, direct by default, and outside release builds", async () => {
    ...
    assert.match(commands, /agent_runtime::resolve\(&app, &candidate\)\.await\?/);
    assert.match(commands, /if settings\.mode == "direct"[\s\S]*use_upstream_proxy: false/);
  ...
  it("isolates ShowNet endpoint, tools, and proxy settings to the launched process", async () => {
    ...
    assert.match(runtime, /configure_os_sandbox\(&mut command\)/);
    assert.match(runtime, /#\[cfg\(any\(target_os = "macos", target_os = "linux"\)\)\]/);
    assert.match(runtime, /\.env_clear\(\)/);
    assert.match(runtime, /\["--deny", "Bash"\]/);
    ...
    assert.match(mcp, /method_available_for_authorization/);
    assert.match(mcp, /"resources\/read"[\s\S]*"prompts\/get"/);
    assert.match(mcp, /validate_analysis_scope_ids/);
    assert.doesNotMatch(await readFile(new URL("../src-tauri/src/agent_tools.rs", import.meta.url), "utf8"), /shownet_eval_scorecard[\s\S]{0,1800}outputPath/);
  });
});
