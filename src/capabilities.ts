...
export const builtInSkillPreview: SkillDefinition[] = [
  ...
  skill(
    ...
    ["读取完整 Hook", "读取完整请求", "读取完整代码", "读取 TLS 指纹 / 出站状态 / PX 证据"],
    [
      "还原明文到密文或签名的转换链",
      "识别算法、密钥来源、随机量和参数排序",
      "用调用证据区分真实算法和相似命名",
      "报告 TLS 保真边界与 PX 结构线索（非硬破）",
    ],
  ...
  ),
  {
    ...skill(
      ...
      "检测到 awswaf、challenge、captcha、telemetry、sensor、_abck、bm_sz、akamai、cf-chl、recaptcha、px 或业务签名参数时启用",
      [
      ...
      ["读取完整请求", "读取完整 Hook", "读取 TLS / 出站状态 / PX 证据", "沙箱 decoder", "机检 scorecard L0/L1/L2"],
      [
        "按提供商、脚本、端点和时序聚合动态防护证据",
        "必须调用 shownet_eval_scorecard；禁止工具失败时虚构满分",
        "有 PX 线索时 list/decode PX 证据（结构解码非硬破）",
        "CAPTCHA 条目计数≠字段级；报告 fidelity 标签与出站 TLS 状态",
        "严格区分已确认、合理推断和本次未捕获项",
      ],
