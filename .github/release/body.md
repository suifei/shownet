...
- **修复空会话无法开始抓包（#12）。** 会话列表为空时，点击「开始抓包」会自动创建并选中新会话，然后立即启动抓包。
- **所有启动入口行为一致。** 顶部按钮、命令面板、内嵌浏览器、代理终端和免代理入口都复用同一条自动创建路径。
- **启动过程不再静默。** 会话加载、自动创建、启动成功和失败都有明确反馈；切换期间会锁定入口，避免双击创建重复会话。

本版本包含 0.4.19 的 Chrome 151 JA4、流式转发、上游代理和 Cloudflare 重试修复。
