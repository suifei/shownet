...
- **Issue 修复。** 修复空白 Request Lab 请求的外键错误、切换会话后请求详情残留、命令窗口焦点漂移、重放压缩响应乱码和离开流量页后筛选丢失，并增加「全部」内容筛选。
- **重放可靠性。** 重放会移除来源请求的 `Accept-Encoding`，并对 gzip、br、deflate、zstd 响应做有上限的解码；不支持的编码会记录明确错误，不再把压缩字节伪装成文本。
- **流量页连续性。** 离开流量页进入 Request Lab 或其他界面时，筛选、排序、选择和详情上下文保持不变；不要求为每个会话新建标签页。
- **启动更新检查。** 桌面版启动后会后台检查新版本并提示用户到设置确认下载；更新不会清除会话、证书、AI、代理或 MCP 配置。
- **强风控登录兼容。** 保留“关闭页面 Hook、继续 MITM 抓包”的通用重试路径，不对缺少域名证据的单一站点添加硬编码绕过。
