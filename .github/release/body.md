...
- **浅色 / 深色 / 跟随系统。** 顶栏、设置、命令面板都能切。浅色用冷银画布和系统蓝主操作，表格和长文本保持实底；深色仍是原来的黑色工作台。默认深色，避免现有用户被系统浅色突然翻盘。
- **README。** 仓库首页换成当前软件的浅色与深色工作台截图，并单独推荐外观切换。
- **Grok 安装兼容性。** 对齐官方 Grok 1.0.5 的 CLI 参数，移除已废弃的 `--no-auto-update`，避免安装成功后被误判为不兼容。

正式包 WebSocket 出站与 HTTPS 走同一套 wreq Chrome TLS。不宣称位级 JA3。本版不加站点白名单，也不关 TLS 解密。
