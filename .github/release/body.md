...
- **AI 失败原因可见。** 分析失败展示模型返回的错误码、类型和说明，不再只显示网关 HTTP 502。
- **失败可改提示词重试。** 保留上次任务提示词，可删减后重试，或换本地模型继续。
- **默认上下文约 100 KiB。** 默认 51,200 token；旧安装未改过的 200K 会读成 51.2K。
- **HTTP/2 Cookie 回送。** 多条 Cookie 头合并后再发给源站。
- **Accept-Language 权重。** 内嵌浏览器不再发出重复 q 值。
- **出站只拨一次。** MITM + wreq 不再先拨号再丢弃。
- **抓包完整性门禁。** Chrome → MITM → wreq 受控链路进入主线。
- **CI 迁到 Node 24 runtime。** 应用构建仍用 Node 22。

WSS 出站身份与 HTTPS 仍不一致（wreq 不能终止 Upgrade），本版不做站点白名单或关闭 TLS 解密。
